...
struct element
{
 ...
 void pqinsert(struct element *h,struct element key,int *count)
 {
  ...
  struct element pqdelete(struct elemen *h, int *count)
  {
    struct element temp;
    temp=h[0];
...
/* void pqinsert(struct element *h,struct element key,int* count)
  {
    ...
    i=*count;
    h[i]=key;//initial position of key
     ++*count;
   ...
   h[i]=key;//insert key in its proper position
 }   
 ...
 void heapify(struct element* h , int count)
 {
   ...
   i=2*j+1;
   //repeat until the left child exists
   while(i<count)
   {
     if((i+1)<count)//check whether the right child exists
     {
       if(h[i+1].pty>h[i].pty)
          i++;//get the smallest child
     }
     if(key.pty < h[i].pty)
     {
   ...
   h[j]=key;//insert key in its proper position
  }
*/
